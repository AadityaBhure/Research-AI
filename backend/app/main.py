...
from app.services.discovery import SemanticScholarService
...
def create_app(settings=None):
    ...
    async def lifespan(app):
        ...
        async with httpx.AsyncClient(timeout=30, follow_redirects=False) as client:
            ...
            app.state.embeddings = EmbeddingService(cfg)
            app.state.discovery = SemanticScholarService(client, cfg, app.state.llm)
            app.state.ingestion = IngestionService(app.state.db, app.state.embeddings, cfg)
            app.state.rag = RAGService(app.state.db, app.state.embeddings, app.state.llm, cfg)
    ...
    async def status(request: Request):
        ...
        return {'status': 'ready', 'llm_provider': 'groq', 'embedding_model': request.app.state.settings.embedding_model}
    ...
    async def save_paper(project_id: UUID, body: PaperInput, request: Request):
        ...
        async with state.ingestion.lock(project_id):
            ...
            result = await state.db.rpc('save_paper', {'target_project_id': str(project_id), 'metadata': body.model_dump(mode='json')})
    ...
    async def upload(project_id: UUID, request: Request, file: UploadFile = File(...),
        ...
        state = request.app.state
        data = await uploaded_bytes(file, state.settings.max_pdf_bytes)
        try:
        ...
        async with state.ingestion.lock(project_id):
            ...
            state.ingestion.schedule(project_id, result['paper']['id'], data)
        return result
    ...
    async def attach_pdf(project_id: UUID, paper_id: UUID, request: Request, file: UploadFile = File(...)):
        ...
        async with state.ingestion.lock(project_id):
            ...
            await state.ingestion.state(str(project_id), str(paper_id), 'saved', ingestion_error=None)
            state.ingestion.schedule(project_id, paper_id, data)
        return {'status': 'saved'}
    ...
    async def retry(project_id: UUID, paper_id: UUID, request: Request):
        ...
        async with state.ingestion.lock(project_id):
            ...
            await state.ingestion.state(str(project_id), str(paper_id), 'saved', ingestion_error=None)
            state.ingestion.schedule(project_id, paper_id)
        return {'status': 'saved'}
