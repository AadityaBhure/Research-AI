...
import { api, safeUrl, type AnswerResponse, type Paper, type Project, type SearchResponse, type Source } from './api';
...
function Workspace({ project, onBack, onEdit }: { project: Project; onBack: () => void; onEdit: () => void }) {
  ...
  async function savePaper(paper: Paper) {
    const key = paper.semantic_scholar_paper_id || paper.title;
    setPending(prev => new Set(prev).add(key)); setError(''); setNotice('');
    try {
      const data = await api<{ status: string; paper: Paper }>(`${base}/papers`, { method: 'POST', body: JSON.stringify(paper) });
      await refresh();
      setNotice(data.status === 'already_exists' ? 'This paper is already in your library.' : data.paper.status === 'no_pdf' ? 'Paper saved. Upload its PDF from the library to make it searchable.' : 'Paper saved. PDF ingestion is starting.');
    } catch (error) { setError(errorText(error)); }
    finally { setPending(prev => { const next = new Set(prev); next.delete(key); return next; }); }
  }
  async function uploadFile(file: File | undefined) {
    if (!file) return;
    if (file.size > 30 * 1024 * 1024 || !file.name.toLowerCase().endsWith('.pdf')) { setError('Choose a PDF smaller than 30 MB.'); return; }
    ...
    try {
      ...
      await refresh(); setNotice(response.status === 'already_exists' ? 'This paper already exists. Use its Attach PDF action if it needs a PDF.' : 'PDF uploaded. Processing has started; watch its status in the library.');
    } catch (error) { setError(errorText(error)); }
  ...
  async function retry(paper: Paper) {
    try { await api(`${base}/papers/${paper.id}/retry`, { method: 'POST' }); await refresh(); }
    catch (error) { setError(errorText(error)); }
  }
  ...
  return <div className="workspace">
    <aside className="library"><button className="back quiet" onClick={onBack}><ArrowLeft size={15} /> All projects</button>
      ...
      <button className="upload-button" disabled={uploading} onClick={() => { attachTarget.current = null; uploadRef.current?.click(); }}><Upload size={16} /> {uploading ? 'Uploading…' : 'Upload a PDF'}<span>+</span></button>
      ...
      <div className="paper-list">{papers.length === 0 ? <div className="library-empty"><FileText size={28} /><p>Your next discovery<br />belongs here.</p><small>Save a search result or upload a PDF.</small></div> : papers.map(paper => <article className="library-paper" key={paper.id}>
        <div className="paper-title-row"><FileText size={16} /><h3>{paper.title}</h3>{paper.status === 'ready' && <input type="checkbox" aria-label={`Focus chat on ${paper.title}`} checked={selected.has(paper.id!)} onChange={e => setSelected(prev => { const next = new Set(prev); if (e.target.checked) next.add(paper.id!); else next.delete(paper.id!); return next; })} />}</div>
    ...
    <main className="conversation"><header className="conversation-header"><div><Sparkles size={17} /><span>Research assistant</span></div><span className="scope-badge">PROJECT KNOWLEDGE</span></header>
      ...
      </article>)}</div>}{busy && <div className="thinking"><LoaderCircle size={15} className="spin" />{mode === 'search' ? 'Finding papers and reading abstracts…' : 'Reading your project’s research…'}</div>}<div ref={bottomRef} /></div>
